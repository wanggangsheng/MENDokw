...
!            end do
    ...
    CASE (2) !read parameter from SCE samples to compute statistics of response variables
        ...
        j = 0
        DO
            read(iFpar, *, iostat = eof) iRead(1:3),rRead(1:3),xx(1:sPAR_SCE%nPar)
            ...
            ELSE
                IF(rRead(1).le.fObj_cr) then  !!fObj = rRead(1)
                    j = j+1
    !                print*, sRead(1:3), rRead,sRead(4:5),xx
                    fObj = fMEND_OBJ(xx, sPAR, sINI, sOUT)
                    write(*,'(A5,I6,A1,2f20.4)')"fObj[",j,"]",rRead(1),fObj
                    write(iFpar_UQ, '(3I6,3f10.4,50f15.8)')iRead(1:3),rRead(1:3),xx
